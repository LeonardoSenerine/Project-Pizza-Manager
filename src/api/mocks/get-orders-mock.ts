...
const orders: Orders = Array.from({ length: 60 }).map((_, i) => {
    return {
        ...
        //basicamente ta pegando o resto da divisao por 5, fica sempre em loop
        status: statuses[i % 5],
        total: 2400,
    }
})
...
export const getOrdersMock = http.get<never, never, GetOrdersResponse>('/orders', async ({ request }) => {
    ...
    //aquele filtrador de opções, para selecionar quais serão aplicados
    if (customerName) {
    ...
    //slice pega um pedaço da aplicação
